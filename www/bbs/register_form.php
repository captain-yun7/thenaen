...
if($exception == 1){
    ...
    $email = $_GET['email'];
    $outs = sql_fetch_array(sql_query("select * FROM g5_memer_temp where email = '$email' order by nums desc limit 0,1"));
    ...
    $height = $outs['height'];
    $weight = $outs['weight'];
    $heart_rate = $outs['heart_rate'];
    $disease = $outs['disease'];
    $nickname = $outs['nickname'];
    $certification = $outs['certification'];

    $varjavauser_nick = $outs['varjavauser_nick'];
    $varjavauser_email = $outs['varjavauser_email'];
    $varjavauser_id = $outs['varjavauser_id'];
    $varjavauser_phone = $outs['varjavauser_phone'];

    $emailCId = str_replace("@", "", $email);
    $emailCId = str_replace(".", "", $emailCId);

    $_POST['mb_id'] = $emailCId;
    $_POST['mb_password'] = $pwds;
    $_POST['mb_name'] = $nickname;
    $_POST['mb_nick'] = $nickname;
    $_POST['mb_email'] = $email;
    $_POST['sex'] = $sex;
    $_POST['age'] = $age;
    $_POST['height'] = $height;
    $_POST['weight'] = $weight;
    $_POST['heart_rate'] = $heart_rate;
    $_POST['disease'] = $disease;
    $_POST['certification'] = $certification;
    $_POST['varjavauser_nick'] = $varjavauser_nick;
    $_POST['varjavauser_email'] = $varjavauser_email;
    $_POST['varjavauser_id'] = $varjavauser_id;
    $_POST['varjavauser_phone'] = $varjavauser_phone;
...
// 그누보드 회원가입 화면 안보이게 처리 ..
echo "
<div style = 'position: absolute;left: 0;top: 0;background-color: white;width: 200%;height: 200%;z-index: 99999;'>
...
if($exception == 1) {
    ...
    $member['mb_password'] = $_POST['mb_password'];
    $member['mb_name'] = $_POST['mb_name'];
    $member['mb_nick'] = $_POST['mb_nick'];
    $member['mb_email'] = $_POST['mb_email'];
    $member['sex'] = $_POST['sex'];
    $member['age'] = $_POST['age'];
    $member['height'] = $_POST['height'];
    $member['weight'] = $_POST['weight'];
    $member['heart_rate'] = $_POST['heart_rate'];
    $member['disease'] = $_POST['disease'];
    $member['certification'] = $_POST['certification'];
    $member['varjavauser_nick'] = $_POST['varjavauser_nick'];
    $member['varjavauser_email'] = $_POST['varjavauser_email'];
    $member['varjavauser_id'] = $_POST['varjavauser_id'];
    $member['varjavauser_phone'] = $_POST['varjavauser_phone'];
