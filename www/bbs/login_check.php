...
if($_POST['exception'] == 1){
    ...
    $mb_id_re = str_replace("@", "", $_POST['mb_id']);
    $mb_id_re = str_replace(".", "", $mb_id_re);

    $_POST['mb_id'] = $mb_id_re; // @ . 문자열 치환

    if($_POST['exceptionSns'] == 1) {

        $_POST_mb_password = my_simple_crypt($_POST['mb_password'], 'd');

        $_POST['mb_password'] = $_POST_mb_password;
