...
$user_email_sns_common = $user_email;



$join_checks_register = sql_fetch_array(sql_query("select count(*) as cnt FROM g5_member where varjavauser_email = '$user_email' and varjavauser_id = '$user_id'"));
...
if($join_checks_register['cnt'] != 0){ // sns 로 회원가입 되어 있으면 자동으로 로그인 시킴
    echo "<script>location.href = '../../login.php?emailgo=$user_email'</script>";
    exit;
}


if($user_email_sns_common != ""){
    echo "<script>location.href = '../../join2.php?user_nick=$user_nick&user_email=$user_email&user_id=$user_id&user_phone=$user_phone'</script>";
    exit;
}
