        ...
        <div class="pd15">
            <div class="i_block">
                ...
                <div class="input_wrap">
                    <input type="number" name="certi_num" id="certi_num" class="input" placeholder="인증코드 6자리를 입력해 주세요." maxlength="6" oninput="num_maxLength(this);">
                    <button type="button" class="c_btn">재전송</button>
                </div>
            </div>

            <!--인증번호 틀리면 red에 addClass('on')-->
                        <div class="red">틀린 인증코드입니다.</div>
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if(result == "false"){
                    alert("인증번호가 맞지 않습니다.");
                    $(".red").addClass("on");
                    TempJoinEmailStrAuthNum = 0;
                }
                else{
                    TempJoinEmailStrAuthNum = 1;

                    if(TempJoinEmailStrAuthNum == 1) {

                        location.href = 'join4.php?email='+getEmail;
                    }
                    else{
